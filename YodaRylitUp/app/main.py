...
def add_news():
    ...
    if form.validate_on_submit():
        ...
        news.img_href = form.img_href.data
...
def news_liked_by(id_user):
    ...
    all_likes = len(liked_list) - 1
    print(liked_list)
    return render_template("liked_news.html", liked_list=liked_list, all_likes=all_likes, news=news)
...
def index():
    ...
    if request.method == 'POST':
        ...
        news_need = list()
        for n in news:
            if find in n.title and cities in n.city:
                print(n.title)
